...
class NetworkHttps {
  ...
  static Future<Map<String, dynamic>> getRequest(String endPoint) async {
    ...
    try {
      var response = await http.get(Uri.parse(getUrl), headers: await _getHeaders());
      ...
      if (!contentType.contains('application/json')) {
        ...
        if (response.statusCode == 405) {
          message = "Method Not Allowed (405). Please contact support or check your request.";
        }
        commonToast(message);
        return {"status": response.statusCode, "message": message, "raw": response.body};
      }
      try {
        if (response.body.isNotEmpty) {
          ...
          responseBody = {"status": response.statusCode, "message": "Empty response body"};
        }
      } catch (e) {
        // If JSON decoding fails, return a structured error with raw status code
        print("Error decoding GET response body: $e");
        responseBody = {"status": response.statusCode, "message": "Invalid JSON response from server: $e"};
      }
      ...
      if (response.statusCode == 401) {
        ...
        return {"status": 401, "message": "Unauthorized. Redirecting to login."};
      } else {
      ...
      return {"status": -3, "message": "An unexpected error occurred: ${e.toString()}"};
    } finally {
      Loader.hideLoader(); // Ensure loader is hidden in all cases
    }
  }

  static Future<Map<String, dynamic>> postRequest(String endPoint, Map data) async {
    ...
    Loader.showLoader(); // Show loader for every API call
    try {
      var response = await http.post(Uri.parse(postUrl), body: jsonEncode(data), headers: await _getHeaders());
      ...
      if (response.statusCode == 401) {
        ...
        if (responseBody['status'] != 1 && responseBody['status'] != 200) {
           commonToast(responseBody['message']?.toString() ?? "An error occurred with status ${response.statusCode}");
        }
        return responseBody;
      }
    } on SocketException {
  ...
  static Future<Map<String, dynamic>> deleteRequest(String endPoint) async {
    ...
    Loader.showLoader(); // Show loader for every API call
    try {
      var response = await http.delete(Uri.parse(deleteUrl), headers: await _getHeaders());
      print("statusCode===> ${response.statusCode}");
      print("response body===> ${response.body}");
