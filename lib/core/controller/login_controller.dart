...
class LoginController extends GetxController {
  ...
  void fetchWorkspaceData() async {
    try {
      ...
      String testToken =
          "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpc3MiOiJodHRwczovL2RvLXN5c3RlbS5jb20vYXBpL0hybS9sb2dpbiIsImlhdCI6MTc1MDU5MTEyNCwiZXhwIjoxNzUwNTk0NzI0LCJuYmYiOjE3NTA1OTExMjQsImp0aSI6IjZyYkxYNVlNb2FMcXRxdjAiLCJzdWIiOiI0NiIsInBydiI6IjIzYmQ1Yzg5NDlmNjAwYWRiMzllNzAxYzQwMDg3MmRiN2E1OTc2ZjcifQ.FTpI73_N7-Kv2GmzO6FB-wvLlPe3Clehpi1RKfJlyMY";
    ...
    try {
      ...
      if (response['status'] == 200) {
        ...
        if (loginResponse.data != null && loginResponse.data!.user != null) {
          ...
          Prefs.setString(
              AppConstant.userName, loginResponse.data!.user!.name ?? "");
          Prefs.setString(
              AppConstant.emailId, loginResponse.data!.user!.email ?? "");
          Prefs.setString(
              AppConstant.phoneNo, loginResponse.data!.user!.mobileNo ?? "");
          Prefs.setString(
              AppConstant.profileImage, loginResponse.data!.user!.avatar ?? "");
          Prefs.setString(
              AppConstant.userType, loginResponse.data!.user!.type ?? "");
          Prefs.setString(AppConstant.workSpaceId,
              loginResponse.data!.user!.activeWorkspace.toString());

          // Store workspaces
          if (loginResponse.data!.workspaces != null) {
            workSpaceList.value = loginResponse.data!.workspaces!;
            Prefs.setString(AppConstant.workSpaceArray,
                jsonEncode(loginResponse.data!.workspaces!));
          }

          Loader.hideLoader();
          Get.offAll(() => const FaceScanScreen());
          commonToast(
              "${loginResponse.data!.user!.name ?? ""} Login successfully");
        } else {
          Loader.hideLoader();
