...
class EventController extends GetxController {
  ...
  String getMonth() {
    ...
    String month = DateFormat('MM')
        .format(now); // 'MM' ensures the month has leading zero if needed
  ...
  fetchEvents(String monthName, String year) async {
    ...
    try {
      var responseData = await NetworkHttps.getRequest(
          "${API.eventCalender}?workspace_id=${Prefs.getString(AppConstant.workSpaceId)}&month=$monthName&year=$year");
      log(responseData['data']['announcements'].toString());
    } catch (e) {
  ...
  void updateSelectedEvents(DateTime date) {
    ...
    log("Calender  Date ${getDateFormmatted(date)}");
    for (var data in eventList) {
      log("appointmentDate${data.startDate}");
    }
    selectedEvents.value = eventList
        .where((event) => event.startDate == getDateFormmatted(date))
        .toList();
  }
}
